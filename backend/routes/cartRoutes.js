...
// @route PUT /api/cart
// @desc Update product quantity in the cart for a guest or user logged-in uer
// acces Public
    ...
    if (guestCart) {
      ...
      if (userCart) {
        ...
        try {
          ...
          return res.status(500).send("Error deleting guest cart");
        }
      ...
      if (userCart) {
        // Guest cart has already been merged, return the guest cart
        return res.status(200).json(userCart);
      } else {
